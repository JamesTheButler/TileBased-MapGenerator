...
public class NoiseBasedTiledMapGenerator : MonoBehaviour {
    ...
    private void Start() {
        ...
        // set tiles
        SetTiles();
        // TODO not good to couple cam object to map generator
        // set up cam 
        if (camMover != null)
    ...
    private void RenderNoise(float[,] noise) {
        ...
        for (int x = 0; x < dimensions.x; x++) {
            for (int y = 0; y < dimensions.y; y++) {
                texture.SetPixel(x, y, ColorFromGreyScale(noise[x, y]));
            }
        }
        texture.Apply();
    ...
    private Color ColorFromGreyScale(float v) {
    ...
    protected void InitTiles() {
        ...
        mountainTiles.Init(new RandomTileIndexer());
    ...
    /// <summary>
    /// Removes small or narrow patches of terrain that would produce artifacts with the tileindexer.
    /// </summary>
